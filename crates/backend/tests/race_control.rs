...
struct SeedMessage<'a> {
    ...
    cleared_by: Option<&'a str>,
}

async fn insert_message(pool: &SqlitePool, seed: SeedMessage<'_>) {
    sqlx::query(
        "INSERT INTO messages (channel, sender, subject, body, ai_category, sentiment, status, received_at, cleared_by) \
         VALUES (?, 'seed', 'subject', 'body', ?, ?, ?, COALESCE(?, strftime('%s','now')), ?)",
    )
    .bind(seed.channel)
    .bind(seed.category.unwrap_or("question"))
    .bind(seed.sentiment.unwrap_or("neutral"))
    .bind(seed.status)
...
async fn stats_aggregate_counts_volumes_and_runner_progress() {
    ...
    insert_player(&pool, "device-alice").await;
