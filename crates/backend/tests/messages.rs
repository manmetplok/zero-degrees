...
use shared::{Channel, ClearResult, CreateHurdleMessage, HurdleMessage, ResponseTimeStats, Urgency};
...
async fn clearing_within_target_awards_full_points_and_a_speed_bonus() {
    ...
    let response = client
        .post(format!("/messages/{}/clear", message.id))
        .dispatch()
        .await;
    assert_eq!(response.status(), Status::Ok);
    let result: ClearResult = response.into_json().await.unwrap();
...
async fn clearing_an_already_cleared_message_returns_conflict() {
    ...
    let path = format!("/messages/{}/clear", message.id);
...
async fn clearing_an_unknown_message_returns_not_found() {
    let client = client().await;
    let response = client.post("/messages/999/clear").dispatch().await;
...
async fn response_time_stats_aggregate_only_cleared_messages_per_urgency() {
    ...
    client
        .post(format!("/messages/{}/clear", on_time.id))
        .dispatch()
        .await;
    client
        .post(format!("/messages/{}/clear", burning.id))
        .dispatch()
        .await;
