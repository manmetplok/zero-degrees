...
pub async fn search(
    ...
    let mut builder = QueryBuilder::<Sqlite>::new(
        "SELECT id, channel, sender, subject, body, received_at, status, \
         COALESCE(manual_category, ai_category) AS category, sentiment, urgency, summary \
    ...
    if let Some(category) = &category {
        builder
            .push(" AND COALESCE(manual_category, ai_category) = ")
