...
pub async fn insert_batch(
    ...
    for message in messages {
        let row = sqlx::query_as::<_, MessageRow>(
            "INSERT INTO messages (channel, sender, subject, body, received_at, status, urgency, sentiment) \
             VALUES (?, ?, ?, ?, ?, 'open', ?, ?) \
...
#[get("/messages")]
pub async fn list(pool: &State<SqlitePool>) -> Result<Json<Vec<SeedMessage>>, Status> {
    let rows = sqlx::query_as::<_, MessageRow>(
        "SELECT id, channel, sender, subject, body, received_at, status, urgency, sentiment, created_at \
         FROM messages ORDER BY received_at",
    )
